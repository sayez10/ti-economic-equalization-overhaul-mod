// SPDX-FileCopyrightText: Copyright © 2022 - 2025 Verdiss, explodoboy, sayez10
//
// SPDX-License-Identifier: MIT

using System;
using HarmonyLib;
using PavonisInteractive.TerraInvicta;



namespace TIEconomyMod
{
    /// <summary>
    /// Patch changes the effects of the spoils investment
    /// This overwrite is necessary to fix the propoganda effect, which would otherwise be far too powerful
    /// Otherwise, this method is almost as vanilla, barring referenced values that are changed in other patches
    ///
    /// nameof() can't be used here because OnSpoilsPriorityComplete is private in vanilla
    /// </summary>
    [HarmonyPatch(typeof(TINationState), "OnSpoilsPriorityComplete")]
    internal static class SpoilsEffectsPatch
    {
        [HarmonyPrefix]
        private static bool OnSpoilsPriorityCompleteOverwrite(TINationState __instance)
        {
            // If mod has been disabled, abort patch and use original method
            if (!Main.enabled) { return true; }

            // As vanilla
            __instance.AddToInequality(__instance.spoilsPriorityInequalityChange, TINationState.InequalityChangeReason.InqReason_SpoilsPriority);
            __instance.AddToDemocracy(__instance.spoilsPriorityDemocracyChange, TINationState.DemocracyChangeReason.DemReason_SpoilsPriority);
            __instance.AddToSustainability(__instance.spoilsSustainabilityChange);

            // Greenhouse Gases
            TIGlobalValuesState.GlobalValues.AddSpoilsPriorityEnvEffect(__instance, 0f);

            // Bonus money for owners of aristocracy and extractive sector control points (if they exist in this nation)
            TIFactionState aristocracyCPOwner = __instance.GetControlPointTypeOwner(ControlPointType.Aristocracy);
            TIFactionState extractiveSectorCPOwner = __instance.GetControlPointTypeOwner(ControlPointType.ExtractiveSector);

            foreach (TIControlPoint iControlPoint in __instance.controlPoints)
            {
                float moneyIncome = __instance.spoilsPriorityMoneyPerControlPoint * ((aristocracyCPOwner == iControlPoint.faction) ? TemplateManager.global.aristoracySpoilsMult : 1f);
                moneyIncome += (extractiveSectorCPOwner == iControlPoint.faction) ? (TemplateManager.global.extractiveSpoilsBonusPerResourceRegion * (float)__instance.currentResourceRegions) : 0f;
                moneyIncome += TIEffectsState.SumEffectsModifiers(Context.SpoilsOutput, iControlPoint.faction, moneyIncome);

                if (iControlPoint.faction != null && !iControlPoint.benefitsDisabled)
                {
                    iControlPoint.faction.AddToCurrentResource(moneyIncome, FactionResource.Money, suppressFactionResourcesUpdatedEvent: false, "Spoils");
                    iControlPoint.faction.thisWeeksCumulativeSpoils += moneyIncome;
                    iControlPoint.faction.thisMonthsCumulativeSpoils += moneyIncome;
                }
            }

            // FIXME: Propaganda effect might need to be re-balanced. Scaling with population?
            const float BASE_PROPAGANDA = -0.125f;
            float propagandaEffect = (__instance.education + __instance.democracy) * BASE_PROPAGANDA;

            foreach (TIFactionState iFaction in __instance.FactionsWithControlPoint)
            {
                __instance.PropagandaOnPop_PerOwnedCPFraction(iFaction.ideology, propagandaEffect);
            }


            return false; // Skip original method
        }
    }
}
